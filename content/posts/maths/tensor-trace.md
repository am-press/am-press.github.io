...
<p>The linear map $\widetilde{\tr}:{\rm Hom}(V, V) \to k$ is defined as </p>
<p>$$\widetilde{\tr} = \tr \circ \Phi^{-1}.$$</p>

### Finite-dimensional vector spaces
<p>If $V$ is a finite vector space with basis $B_V = \{e_1, \ldots, e_n\}$, then the dual basis, of $V^*$, is $B_{V^*} = \{e_1^*, \ldots, e_n^*\}$. The tensor space has basis vectors $B_{V^*\otimes V} = \{e_i^* \otimes e_j\}_{i,j=1}^{n}$ and $\dim (V^*\otimes V) = n^2$. The trace has been defined as a linear map $\tr: V^* \otimes V\to k$, with $\tr(a\otimes u) = a(u)$, so it suffices to see how it works on the basis of the tensor space. It is </p>
<p>$$\tr(e_i^*\otimes e_j) = e_i^*(e_j) = \delta_{i,j},$$</p>
